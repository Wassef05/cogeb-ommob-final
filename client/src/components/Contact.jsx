...
const Contact = ({ listing }) => {
    ...
    useEffect(() => {
        (async () => {
            setLoading(true)
            const res = await fetch(`http://localhost:4000/api/users/${listing.userRef}`)
    ...
    const handleSendMsg = async () => {
        ...
        try {
            ...
            if (json.success === false) {
                ...
                const resMsg = await fetch("http://localhost:4000/api/message/create", {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(
                        {
                        ...
                        <div className="property_owner mt-10">
                            <div className="image_container flex items-center justify-start gap-2">
                                ...
                                <div className="tittle">
                                    ...
                                    <p className='font-content font-bold ,text-sm text-gray-500'>Propriétaire</p>
                                </div>
                            </div>
                        </div>
                        {
